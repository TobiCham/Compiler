...
internal object TypeMerger {
    ...
    private fun canInvoke(function: FunctionType, args: List<ExpandedType>): Boolean = when(function.parameters) {
        ...
        is KnownParameters -> {
            val params = (function.parameters as KnownParameters).params
    ...
    fun mergeTypes(type1: ExpandedType, type2: ExpandedType): ExpandedType {
        ...
        if(type1 is FunctionType && type2 is FunctionType) {
            ...
            if(type1.parameters is KnownParameters && type2.parameters is KnownParameters && (type1.parameters as KnownParameters).size == (type2.parameters as KnownParameters).size && type2.returnType is FunctionType) {
                val type1Params = (type1.parameters as KnownParameters).params
                val type2Params = (type2.parameters as KnownParameters).params
                ...
                if(type1Params == type2Params) {
                    ...
                    val params = List((type1.parameters as KnownParameters).size) {
                        mergeTypes(
                            type1Params[it],
                            type2Params[it]
                        )
