...
object TypeMerger {
    ...
    fun canBeAssignedTo(expectedType: ExpandedType, actualType: ExpandedType): Boolean = when {
        ...
        actualType is IntersectionType -> actualType.possible.any { canBeAssignedTo(expectedType, it) }
        expectedType is IntersectionType -> expectedType.possible.any { canBeAssignedTo(it, actualType) }
    ...
    fun restrictType(type: ExpandedType, restrictTo: DataType): ExpandedType {
        if(type is IntersectionType) {
            ...
            return if(newTypes.size == 1) newTypes.first() else IntersectionType(newTypes.toSet())
        }
    ...
    fun canBeAssignedToStrict(expectedType: ExpandedType, actualType: ExpandedType): Boolean {
        when {
            ...
            actualType is IntersectionType -> return actualType.possible.all { canBeAssignedToStrict(expectedType, it) }
            expectedType is IntersectionType -> return expectedType.possible.any { canBeAssignedToStrict(it, actualType) }
            expectedType is FunctionType || actualType is FunctionType -> {
                ...
                if (!canBeAssignedToStrict(expectedType.returnType, actualType.returnType)) return false
                ...
                return expectedParams.params.size == actualParams.params.size && expectedParams.params.withIndex().all { (i, value) ->
                    canBeAssignedToStrict(value, expectedParams.params[i])
                }
            }
            else -> return false
        }
    ...
    fun mergeTypes(type1: ExpandedType, type2: ExpandedType): ExpandedType {
        ...
        if(type1 is FunctionType && type2 is FunctionType) {
            ...
            if(type1.parameters == type2.parameters) {
                return FunctionType(mergeTypes(type1.returnType, type2.returnType), type1.parameters)
            }
            ...
            if(type1.parameters is KnownParameters && type2.parameters is KnownParameters && type1.parameters.size == type2.parameters.size && type2.returnType is FunctionType) {
                ...
                if(type1Params == type2Params) {
                    ...
                    return FunctionType(returnType, KnownParameters(params))
                }
            }
        }
    ...
    fun getSimpleType(type: ExpandedType): DataType? = when (type) {
        ...
        is IntersectionType -> {
            val types = type.possible.map { getSimpleType(it) }.filterNotNull().toSet()
            if(types.size == 1) types.first() else null
        }
        else -> null
    }

    fun <T> getNewTypeFromAssignment(existingType: T, newType: ExpandedType): T where T : ExpandedType, T : CompleteType {
        if(newType !is CompleteType) {
            return existingType
        }
        return mergeTypes(existingType, newType) as T
    }
}